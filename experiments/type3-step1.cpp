...
int main(int argc, char* argv[]) {
    ...
    /* Arb b1(TYPE_3_B1_HARD);
    Arb b2(-0.2);
    Arb rho = Config::rho_safe(b1, b2, -1 + Arb::abs(b1+b2));
    Arb beta(TYPE_3_BETA);
    Arb eps(0.0000001);

    Arb ob = obj(b1, b2, rho, beta);
    ob.println();
    Arb ob_d_b1_est = (obj(b1+eps, b2, rho, beta) - obj(b1, b2, rho, beta)) / eps;
    Arb ob_d_b2_est = (obj(b1, b2+eps, rho, beta) - obj(b1, b2, rho, beta)) / eps;
    Arb ob_d_rho_est = (obj(b1, b2, rho+eps, beta) - obj(b1, b2, rho, beta)) / eps;

    ob_d_b1_est.println();
    obj_d_b1(b1, b2, rho, beta).println();

    ob_d_b2_est.println();
    obj_d_b2(b1, b2, rho, beta).println();

    ob_d_rho_est.println();
    obj_d_rho(b1, b2, rho,beta).println(); */

    Arb b1_range(-1, 1);
    Arb b2_range(-1, 1);
    Arb rho_range(-1, 1);
    Arb beta_range(TYPE_3_COARSE_BETA_LO, TYPE_3_COARSE_BETA_HI); 
    ...
    flint_printf("rho: "); rho_range.pretty_println();
